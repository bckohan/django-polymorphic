...
if rdbms == "sqlite":  # pragma: no cover
    ...
    dbs = os.environ.get("ORACLE_DATABASE", "test1,test2").split(",")
    creds = {
        "USER": os.environ.get("ORACLE_USER", "system"),
        "PASSWORD": os.environ.get("ORACLE_PASSWORD", "password"),
    }
    db_name = (
        f"{os.environ.get('ORACLE_HOST', 'localhost')}:{os.environ.get('ORACLE_PORT', '1521')}"
    )
    DATABASES = {
        "default": {"ENGINE": "django.db.backends.oracle", "NAME": f"{db_name}/{dbs[0]}", **creds},
        "secondary": {
            "ENGINE": "django.db.backends.oracle",
            "NAME": f"{db_name}/{dbs[1]}",
            **creds,
        },
    }
